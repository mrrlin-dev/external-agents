...
const RECORDED_ENVELOPES = {
  ...
  // Captured live 2026-09-10 against agy 1.2.0 (`agy --output-format json
  // --model gemini-3.6-flash-low --print "Create a file named hello.txt
  // containing exactly the text: hello world"`) while verifying the fix for
  // the print-timeout false-success class: a single JSON object, not an event
  // array, the answer under `.response`, no cost field at all.
  agy: {
    stdout: JSON.stringify({
      conversation_id: "e13be7ad-7a7c-442a-a5a6-cbd32842bbee",
      status: "SUCCESS",
      response: "I have created the file [hello.txt](file:///tmp/agy-json-test/hello.txt) with the exact content `hello world`.\n",
      duration_seconds: 9.486174,
      num_turns: 1,
      usage: { input_tokens: 15406, output_tokens: 272, thinking_tokens: 0, cache_read_tokens: 44698, total_tokens: 15678 },
    }),
    expect: {
      text: "I have created the file [hello.txt](file:///tmp/agy-json-test/hello.txt) with the exact content `hello world`.\n",
      tokens_in: 15406,
      tokens_out: 272,
      cache_read: 44698,
    },
