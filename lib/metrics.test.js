...
// --- the tier split -------------------------------------------------------
//
// A decomposition, not a coefficient. A weak seat's token is plainly not worth
// a strong seat's, but the fix for that is to show both and let the reader
// price the difference — not to fold somebody's guess into one figure.

const TIERED = [
  ["s1", { count: 3, tokens_in: 1e6, tokens_out: 1e6 }],
  ["w1", { count: 2, tokens_in: 1e6, tokens_out: 0 }],
];
const tierOf = (id) => (id.startsWith("w") ? "weak" : "strong");

test("by_tier splits the same totals and never invents a number", () => {
  const e = savedEstimate(TIERED, ANCHOR, tierOf);
  assert.ok(e.by_tier, "the split must be present when a tier lookup is given");
  assert.equal(e.by_tier.strong.dispatches_free, 3);
  assert.equal(e.by_tier.weak.dispatches_free, 2);
  // The two halves are exactly the whole — a coefficient would break this.
  assert.equal(e.by_tier.strong.tokens_free + e.by_tier.weak.tokens_free, e.tokens_free);
  assert.equal(
    e.by_tier.strong.tokens_free_billable + e.by_tier.weak.tokens_free_billable,
    e.tokens_free_billable,
  );
});

test("each tier is priced by the same two methods as the whole", () => {
  const e = savedEstimate(TIERED, ANCHOR, tierOf);
  // weak is 1M input only: blended $3 vs split $2, so the split is the LOW one.
  assert.equal(e.by_tier.weak.saved_low, 2);
  assert.equal(e.by_tier.weak.saved_high, 3);
  // strong is 1M in + 1M out: blended $6 vs split $12.
  assert.equal(e.by_tier.strong.saved_low, 6);
  assert.equal(e.by_tier.strong.saved_high, 12);
});

test("no tier lookup means no split — the tile that does not ask does not get one", () => {
  assert.equal(savedEstimate(TIERED, ANCHOR).by_tier, undefined);
});

test("a missing or unknown tier lands in strong, which changes no total", () => {
  const e = savedEstimate(
    [["a", { count: 1, tokens_in: 1e6 }], ["b", { count: 1, tokens_in: 1e6 }]],
    ANCHOR,
    (id) => (id === "a" ? undefined : "nonsense"),
  );
  assert.equal(e.by_tier.weak.dispatches_free, 0);
  assert.equal(e.by_tier.strong.dispatches_free, 2, "unclassified seats get the full rate, as before");
  assert.equal(e.by_tier.strong.saved_low + e.by_tier.weak.saved_low, e.saved_low);
});
