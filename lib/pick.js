...
import { effectiveTokenCeiling, budgetBlocks } from "./budget.js";
import { quarantineReason, healthBand } from "./outcome.js";
...
// The registry's declared token_limits for `entry`'s voice: its own, or (a
// numbered key declares none of its own) the first same-voice sibling's. Same
// rule pickAgents applies inline for every candidate at once — pulled out so a
// single dispatch can ask the same question about one entry without pick.
export function inheritedTokenLimits(registry, entry) {
  if (entry.token_limits) return entry.token_limits;
  const voice = agentVoice(entry);
  for (const other of registry.agents) {
    if (other.token_limits && agentVoice(other) === voice) return other.token_limits;
  }
  return null;
}
...
// Bytes→tokens for the size gate. 3.5, not 4, and the difference is the whole
// point: a gate needs an upper bound on the token count, and 4:1 was the MEDIAN.
//
// Measured over 4004 successful dispatches that reported both prompt_bytes and
// tokens_in: median 3.95, p10 3.60, p5 3.51, p1 3.20, min 2.44. Half of all real
// prompts are denser than 4 bytes/token, so half the time the old estimate ran
// under the truth — and a size gate that under-counts does not refuse anything,
// it just lets the provider do the refusing with an HTTP 413.
//
// That is not hypothetical. groq-gpt-oss-120b-2 returned 413 on a 28,930-byte
// prompt against its measured 8000-token ceiling: 4:1 called that 7233 tokens,
// comfortably under the cap, so nothing stopped it. At 3.5 the same bytes
// estimate 8266 and the seat is correctly skipped.
//
// p5 rather than p1 or the minimum on purpose. The error is asymmetric but not
// one-sided: too generous burns a dispatch on a 413, too strict silently walks
// past allowance the seat would have served, with nothing failing to point at
// it. p5 covers 95% of observed prompts and still costs ~12% of nominal
// headroom; p1 (3.20) would cost 25% to buy the last 4 points of coverage.
// Callers that know their real token count pass prompt_tokens and skip all this.
export const BYTES_PER_TOKEN = 3.5;

export const estimateTokens = (filter) =>
  Number.isFinite(filter.prompt_tokens)
    ? filter.prompt_tokens
    : Number.isFinite(filter.prompt_bytes)
    ? Math.ceil(filter.prompt_bytes / BYTES_PER_TOKEN)
    : null;
...
export function pickAgents(registry, state, opts = {}) {
  ...
  const estimatedTokens = estimateTokens(filter);
  ...
  const candidates = registry.agents.filter((entry) => {
    ...
    // Never once answered, in QUARANTINE_AFTER_ATTEMPTS tries. Not a cooldown —
    // there is nothing to wait out. Measured: openrouter-gemma-4-31b-free was
    // seated 73 times over 40 days and failed 73 times, because a cooldown
    // lapsing was the only thing the pool ever remembered about it.
    //
    // The bar is "has NEVER succeeded", deliberately, so this can never mistake
    // an exhausted free tier for a dead model: anything that has ever worked is
    // ordered lower (healthBand, below) but stays available. `audit` clears it.
    if (quarantineReason(rec)) return false;
    ...
    const inherited = limitsByVoice.get(agentVoice(entry));
    if (estimatedTokens != null) {
      // Observation before declaration. The registry's number is somebody's note
      // about an account; a response header is a measurement of THIS key. Both
      // directions of error were measured: azure-kimi-k2-5-safe declared nothing
      // and really has a 5000-token ceiling (45 impossible dispatches), while
      // groq-llama-3.3-70b succeeded 59 times above the 8000 its family was
      // assumed to share. One rule gets both right; a hand-maintained constant
      // gets at most one.
      const { tpm } = effectiveTokenCeiling(entry, rec, inherited, now);
      if (tpm != null && tpm < estimatedTokens) return false;
      // Context window stays declaration-only: no provider reports it in a
      // header, so there is nothing to observe and silence here means unknown.
      const declared = entry.token_limits || inherited;
      if (declared && Number.isFinite(declared.context_window) && declared.context_window < estimatedTokens) return false;
    }
    // Checked whether or not the caller gave a size: "0 requests left in this
    // window" disqualifies a seat on its own, and that is the openrouter
    // account-wide free cap in one line.
    if (budgetBlocks(rec, estimatedTokens, now)) return false;
  ...
  candidates.sort((a, b) => {
    ...
    // Health outranks least-recently-used, and both outrank nothing else.
    //
    // `last_used_at` is stamped at dispatch START, so LRU is honest about
    // attempts — but it is blind to outcomes, and failures come back fast: 14.7s
    // mean against 47.6s for a success. A broken seat therefore returned to the
    // front of this queue roughly three times quicker than a working one, so LRU
    // alone did not merely tolerate dead agents, it over-sampled them.
    //
    // The band is coarse (four buckets) on purpose. A finer score would override
    // the LRU tiebreak entirely and collapse "spread across the live models of a
    // tier" into "whichever model is marginally luckiest gets all the traffic".
    const ha = healthBand(state[a.id]);
    const hb = healthBand(state[b.id]);
...
/**
 * Why did a pick come back empty?
 *
 * `pick` exiting 3 with no output is honest but useless: "everything is cooled
 * down", "everything is quarantined", and "your prompt is bigger than every seat
 * you have" are three completely different problems with three different
 * remedies, and they were indistinguishable.
 *
 * Raised in review as "the quarantine filter can empty the pool". It can — and so
 * could the cooldown filter long before any of this, so an empty pool is a state
 * this function has to be able to DESCRIBE rather than one it should paper over
 * by re-offering seats that have failed eight times running. A caller that is
 * told which filter emptied the set can act; one that is handed a bare exit code
 * cannot.
 *
 * Counts each entry against the FIRST reason it was rejected, in the same order
 * `pickAgents` applies them, so the tally reads as a funnel. Called only on the
 * empty path, so it costs nothing in the normal case.
 */
export function explainEmptyPick(registry, state, opts = {}) {
  const filter = opts.filter || {};
  const now = Math.floor(Date.now() / 1000);
  const estimatedTokens = estimateTokens(filter);
  ...
  for (const entry of registry.agents) {
    const rec = state[entry.id];
    if (!isAgentEnabled(entry, state)) { bump("switched off"); continue; }
    const entryState = rec?.state;
    if (entryState && entryState !== "healthy") {
      const expiresAt = effectiveCooldownUntil(rec);
      if (!(expiresAt != null && now >= expiresAt)) { bump(`cooling down (${entryState})`); continue; }
    }
    if (quarantineReason(rec)) { bump("quarantined (never answered)"); continue; }
    if (filter.tier && entry.tier !== filter.tier) { bump(`wrong tier (not ${filter.tier})`); continue; }
    if (estimatedTokens != null) {
      const { tpm } = effectiveTokenCeiling(entry, rec, limitsByVoice.get(agentVoice(entry)), now);
      if (tpm != null && tpm < estimatedTokens) { bump(`prompt too large (needs ${estimatedTokens} tokens)`); continue; }
    }
    if (budgetBlocks(rec, estimatedTokens, now)) { bump("no budget left this window"); continue; }
    bump("excluded by transport, tags, effort or an explicit --exclude");
  }
...
/** One line, for stderr, when a pick returns nothing. */
export function formatEmptyPick(explain) {
  const parts = Object.entries(explain.reasons).map(([why, n]) => `${n} ${why}`);
  return `pick: no candidates out of ${explain.total} entries — ${parts.join("; ")}`;
}
