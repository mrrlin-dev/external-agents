...
// This file drives runAny, and runAny always appends an aggregate row. Point it
// at a scratch file so a suite run adds nothing to the operator's real log —
// the rejection path below is not supposed to reach the write, and this is what
// keeps that true if it ever starts to.
process.env.EXTERNAL_AGENTS_DISPATCH_LOG_FILE = path.join(
  fs.mkdtempSync(path.join(os.tmpdir(), "ea-dl-files-")),
  "dispatch-log.jsonl",
);
