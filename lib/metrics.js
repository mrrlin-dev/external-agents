 ...
 */
export function savedEstimate(freeAgents, anchor) {
  const entries = Array.isArray(freeAgents) ? freeAgents : [];
  let dispatches = 0, tokens = 0, billable = 0, billableIn = 0, out = 0;
  for (const [, a] of entries) {
    if (!a) continue;
    dispatches += a.count || 0;
    tokens += totalTokens(a);
    billable += billableTokens(a);
    // Input side only, cache tiers already weighted. Reusing billableTokens
    // with tokens_out omitted keeps one definition of "what input costs" —
    // duplicating the multipliers here is how the two halves drift apart.
    billableIn += billableTokens({ tokens_in: a.tokens_in, cache_read: a.cache_read, cache_write: a.cache_write });
    out += a.tokens_out || 0;
  }
  const M = 1_000_000;
  ...
  const low = (billable / M) * anchor.blended_per_m;
  const high = (billableIn / M) * anchor.input_per_m + (out / M) * anchor.output_per_m;
  return {
    dispatches_free: dispatches,
    tokens_free: tokens,
    tokens_free_billable: billable,
