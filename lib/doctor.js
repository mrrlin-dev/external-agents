...
export function runChecks({ rows, registry, state, since, now }) {
  ...
  // The quarantine rule exists to make this impossible, so a seat that answered
  // nothing all window is worth surfacing whatever the cause. What this check
  // must NOT do is guess the cause, and it used to: every non-quarantined case
  // was reported as "a regression in the counters or in the pick filter, not a
  // provider problem".
  //
  // That sentence was measured false the first time it fired unattended. The
  // seat had 11 dispatches and 0 successes; running the check's own remedy
  // returned HTTP 429 `free-models-per-day` — an account-wide free tier that had
  // simply run out, which is ordinary and documented. The counters were fine.
  //
  // A free tier running out is a DAILY event once this runs on a schedule, so a
  // report that calls it a regression every morning is the noise this file's
  // header warns about. The state written by the last audit already knows the
  // difference, so the cause is read from it instead of asserted.
  const SUPPLY_STATES = new Set(["rate_limited", "quota_exhausted"]);
  const ACTIONABLE_STATES = new Set(["needs_auth", "model_unavailable", "not_installed"]);
  ...
  for (const a of neverAnswered) {
    const rec = state[a.id] || {};
    const quarantined = Boolean(quarantineReason(rec));
    const evidence = [`attempts=${a.n}`, "successes=0", `quarantined=${quarantined}`, `state=${rec.state ?? "unknown"}`];
    if (Number.isFinite(rec.cooldown_until)) {
      evidence.push(`cooldown_until=${new Date(rec.cooldown_until * 1000).toISOString()}`);
    }

    let severity, detail, remedy;
    if (quarantined) {
      severity = "low";
      detail = "Already quarantined, so the mechanism worked — these are the attempts it took to "
        + "reach the bar. Nothing to do unless the count keeps climbing.";
      remedy = null;
    } else if (SUPPLY_STATES.has(rec.state)) {
      severity = "low";
      detail = `The provider says this seat is ${rec.state}, so the failures are supply and not a `
        + "defect: `pick` skips it until the cooldown above expires and it comes back on its own. "
        + "Worth a second look only if it is a PAID seat, or if the same seat is here every day — "
        + "a free tier that is always empty is a seat the pool should stop counting on.";
      remedy = null;
    } else if (ACTIONABLE_STATES.has(rec.state)) {
      severity = "high";
      detail = `The last probe left this seat \`${rec.state}\`, which is a configuration problem `
        + "rather than a busy provider: it will keep being seated and keep failing until somebody "
        + "fixes the credential or removes the entry.";
      remedy = `external-agents audit --provider ${providerFamilyOf(rec.provider) || "<provider>"}`;
    } else {
      // Only here does the original diagnosis actually hold: nothing about the
      // seat's recorded state explains why every dispatch failed.
      severity = "high";
      detail = `Nothing in this seat's recorded state (\`${rec.state ?? "unknown"}\`) explains a whole `
        + "window of failures, and it was not quarantined either. The health counters are the only "
        + "thing standing between this and the 73-failures-over-40-days pattern, so this is where "
        + "to look for a regression in the counters or in the pick filter. Run the audit first: if "
        + "it comes back rate_limited or quota_exhausted, the answer was supply and this seat's "
        + "state was simply stale.";
      remedy = `external-agents audit; then check state.json .["${a.id}"].health`;
    }

    add({ id: "never_answered", goal: 1, severity, summary: `${a.id}: ${a.n} dispatches, 0 successes in the last ${windowHours}h`, detail, remedy, evidence });
  }

  // ---- goal 2, leading indicator: seats with no ceiling -------------------
  // This is the check that would have caught a 5000-token seat on day one
  // instead of after 45 impossible dispatches. A seat with no ceiling is not
  // broken; it is a seat `pick` cannot protect, which is strictly worse than a
  // small one.
  //
  // Two findings, not one, because they have different remedies and only one of
  // them is achievable. A seat nobody has probed is fixed by probing it. A seat
  // whose provider answers without any rate-limit headers cannot be fixed that
  // way at all, and telling the operator to run `audit` again every morning is
  // the noise this file's header warns about — a check that cannot say what to
  // do. Measured: one provider in this pool returns 23 headers and not one
  // `x-ratelimit-*`, while the same probe reads a ceiling off two others.
  const unmeasured = [];
  const unmeasurable = [];
  ...
  for (const e of registry.agents) {
    ...
    if (tpm != null || Number.isFinite(ctx)) continue;
    // The provider has answered at least once and reported nothing.
    if (state[e.id]?.limits_unreported) unmeasurable.push(`${e.id} [${e.provider}]`);
    else unmeasured.push(`${e.id} [${e.provider}] (${source})`);
  }
  if (unmeasured.length > 0) {
    add({
      ...
      goal: 2,
      severity: "medium",
      summary: `${unmeasured.length} enabled HTTP seat(s) have no token ceiling and have never been probed`,
      detail:
  ...
  if (unmeasurable.length > 0) {
    add({
      id: "unmeasurable_seat",
      goal: 2,
      // Lower than `unmeasured_seat` on purpose: this one has already had the
      // only automatic fix applied to it and it did not take. Repeating it at
      // medium every morning would train the reader to close the report.
      severity: "low",
      summary: `${unmeasurable.length} enabled HTTP seat(s) have a provider that reports no rate-limit headers`,
      detail:
        "These have been probed and the provider answered without a single rate-limit header, "
        + "so no amount of auditing will ever produce a ceiling for them. `pick` cannot size a "
        + "prompt against them and will seat anything. The only fix is a hand-written "
        + "`token_limits` on the registry entry — and only if you have a number worth trusting: "
        + "a guessed ceiling that is too low silently discards the rest of the allowance with "
        + "nothing failing to point at it. A seat that has never actually been rejected is "
        + "fine to leave exactly as it is.",
      remedy: "declare token_limits on the entry, or accept that this seat is unsized",
