...
import { agentVoice, isAgentEnabled } from "./pick.js";

const STATE_DIR = path.join(os.homedir(), ".local", "state", "external-agents");
export const DISPATCH_LOG = path.join(STATE_DIR, "dispatch-log.jsonl");

// Test fixtures dispatch through the real path and fail on purpose. Counting
// them makes every threshold below meaningless — measured: they contributed 174
// guaranteed failures to a 1472-failure total.
const FIXTURE = /^test-/;

/** Read the dispatch log, newest-first is not needed — callers filter by ts. */
export function readDispatchRows(file = DISPATCH_LOG) {
  if (!fs.existsSync(file)) return [];
  const rows = [];
  for (const line of fs.readFileSync(file, "utf-8").split("\n")) {
    if (!line.trim()) continue;
    try { rows.push(JSON.parse(line)); } catch { /* a torn last line is not a finding */ }
  }
  return rows;
}

export const providerFamilyOf = (p) => String(p || "").replace(/\d+$/, "");
