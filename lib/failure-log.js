...
//
// The list is names, so it is only ever as complete as the conventions someone
// thought of. A consensus round measured three more that returned false against
// live values: GH_PAT / GITHUB_PAT (a fine-grained github_pat_… token matched no
// pattern either, so it passed all three passes in the clear), MY_PSK, and
// DATABASE_URL. PAT and PSK are added here as whole segments — PAT is safe as a
// SEGMENT and must never join the substring list below, where it would swallow
// PATH. A credential embedded in a URL is caught by shape instead, in `redact`,
// because the variable holding a connection string is named after the database
// far more often than after the password inside it.
...
// PSK joins this list (MYPSK is one word to the splitter); PAT deliberately does
// NOT — as a substring it swallows PATH.
const SECRET_NAME_SUBSTRINGS = ["APIKEY", "SECRET", "PASSWORD", "PASSWD", "PASSPHRASE", "CREDENTIAL", "TOKEN", "PSK"];
...
export function redact(text, env = process.env) {
  ...
  for (const [name, value] of secretsFromEnv(env)) {
    ...
    // The backstop pass runs over the SERIALISED line, where a secret holding a
    // quote, a backslash or a newline no longer appears literally — JSON has
    // escaped it, so the pattern above walks straight past. Blank the escaped
    // form too, for the fields this module does not model and cannot redact
    // before serialisation.
    const escaped = JSON.stringify(value).slice(1, -1);
    if (escaped !== value) {
      out = out.replace(new RegExp(escapeRegExp(escaped), "g"), `«redacted:${name}»`);
    }
  }
  out = out
    ...
    // Fine-grained GitHub tokens do not start with ghp_, and the name they are
    // usually held under (GH_PAT) was not a secret name until this round.
    .replace(/\bgithub_pat_[A-Za-z0-9_]{20,}/g, "«redacted:key-like»")
    // A password inside a connection string. Shape, not name: DATABASE_URL,
    // REDIS_URL and AMQP_URL are all named after the service, so the value layer
    // never sees them. Keeps the scheme, user and host — the diagnostic half —
    // and blanks only what sits between the colon and the @.
    .replace(/\b([a-z][a-z0-9+.-]*:\/\/[^\s:/@]+):([^\s@]{1,512})@/gi, "$1:«redacted:url-credentials»@")
...
export function recordFailure(record, options = {}) {
  ...
  try {
    ...
    // exists keeps whatever mode it has — including one an earlier version, an
    // operator, or a redirected EXTERNAL_AGENTS_FAILURE_LOG_FILE left wider than
    // 0600. One stat on the failure path is cheap next to being wrong about who
    // can read this file.
    try {
