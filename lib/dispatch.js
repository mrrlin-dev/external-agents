...
export function buildAiderIgnoreFile(workdir, absFiles, historyDir) {
  ...
  // that file from the chat.
  for (const abs of absFiles) {
    if (abs !== gitRoot && !abs.startsWith(gitRoot + path.sep)) return null;
  }

  // With no declared files the ignore file is still written, deliberately: it
  // turns "aider quietly picked its own files out of the prompt" into an
  // explicit empty run, which the emptyRunReason/emptyRunExitCode path already
  // reports honestly.
  const lines = ["*"];
  for (const abs of absFiles) {
