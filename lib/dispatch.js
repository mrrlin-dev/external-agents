...
export function aiderExitCode(code, isAider, stdout, files) {
  if (!isAider || code !== 0) return code;
  if (files.length > 0) return code;
  return AIDER_PROVIDER_ERROR_RE.test(stdout) ? 1 : code;
}

/**
 * A CLI dispatch exists to return text or to change files. `kiro` exits 0
 * having done neither and printed nothing at all — reported verbatim that is
 * `outcome: success` for work that never happened, the same silent-success
 * class the read_only axis was introduced to stop.
 *
 * Applies to every CLI transport, not just aider. Whitespace-only stdout
 * counts as empty; anything the agent actually said is left alone, however
 * short, because "no" is a legitimate answer.
 */
export function emptyRunExitCode(code, stdout, files) {
  if (code !== 0) return code;
  if (files.length > 0) return code;
  return stdout.trim() === "" ? 1 : code;
}
    ...
    child.on("close", (code) => {
      ...
      resolve({
        ...
        exitCode: timedOut
          ? 124
          : emptyRunExitCode(
              aiderExitCode(code, isAiderCommand(parts), cleanStdout, files),
              cleanStdout,
              files,
            ),
...
export function selectTransport(agentEntry, options = {}) {
  ...
  if (forced === "read_only") {
    if (getTransportConfig(agentEntry, "read_only")) return "read_only";
    // generate_new has no filesystem access at all, so it is read-only by
    // construction — it satisfies the request without declaring anything.
    if (getTransportConfig(agentEntry, "generate_new")) return "generate_new";
    throw new Error(
      `runAny: transport 'read_only' requested but ${agentEntry?.id} declares neither 'read_only' nor 'generate_new' — refusing to fall back to a write-capable transport`,
    );
  }
...
export async function probeReadOnlyNonWriting(entry) {
  const cmd = getTransportConfig(entry, "read_only")?.cmd;
