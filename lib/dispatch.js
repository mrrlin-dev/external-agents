...
export async function verifyCredential(agentEntry) {
  ...
  try {
    ...
    // 429 rate limit — try the standard Retry-After header first (most
    // precise), fall back to scanning the body text for the same reset
    // patterns parseExhaustionSignal already knows how to read.
    if (resp.status === 429) {
      const retryAfterHeader = resp.headers.get("retry-after");
      const retryAfterSeconds = retryAfterHeader && /^\d+$/.test(retryAfterHeader.trim())
        ? parseInt(retryAfterHeader.trim(), 10)
        : null;
      const reset_at = retryAfterSeconds != null
        ? Math.floor(Date.now() / 1000) + retryAfterSeconds
        : parseExhaustionSignal(text).reset_at;
      return {
        ok: false,
        status: resp.status,
    ...
    child.on("close", (code) => {
      ...
      if (isQuota) {
        return resolve({
          ...
          latencyMs,
          reset_at: parseExhaustionSignal(preview).reset_at,
        });
      }
