...
// something destructive. Zero is honoured if asked for explicitly, but is not
// advertised anywhere: with a zero window the cutoff is "now", so it would
// collect the workdir of a dispatch running this second.
...
export function sweepDispatchTemp(options = {}) {
  ...
  const cutoff = (options.now ?? Date.now()) - days * 86400_000;
...
export async function auditCliEntry(entry) {
  ...
  return new Promise((resolve) => {
    const child = spawn("bash", ["-c", `${cmd} ${promptFlag}"reply exactly OK"`], {
      stdio: ["ignore", "pipe", "pipe"],
      env: process.env,
    });
    let out = "", err = "";
    ...
    child.on("close", (code) => {
      clearTimeout(timer);
      const latencyMs = Date.now() - start;
      if (timedOut) return resolve({ ok: false, hint: `timed out after ${Math.round(latencyMs / 1000)}s waiting for a reply`, latencyMs });
      ...
      resolve({ ok: false, hint: (_lastMeaningfulLine(preview) || `exit ${code}`).slice(0, 200), latencyMs });
    });
  });
}
