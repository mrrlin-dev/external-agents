...
import { appendDispatchRow, readDispatchRows } from "./dispatch-log.js";
import { parseCliUsage } from "./cli-usage.js";
import { effectiveTokenCeiling } from "./budget.js";
import { readState } from "./state.js";

// Bytes→tokens at 4:1 — the same deliberately crude estimate `pick --prompt-bytes`
// uses (lib/pick.js), so a prompt judged "too big" here is judged the same way
// there.
const PROMPT_BYTES_PER_TOKEN = 4;

/**
 * Would this prompt overflow `agentEntry`'s known token-per-minute ceiling?
 * Returns a human-readable refusal reason, or null if it fits (including when
 * no ceiling is known at all — silence is not a refusal, see effectiveTokenCeiling).
 *
 * `pick` has done this check since the registry gained `token_limits` (#51):
 * seat only an agent whose declared/observed ceiling can hold the prompt. But
 * `dispatch` — reachable directly, by any caller that names an agent id and
 * skips pick — never checked it, so a caller that forgets `--prompt-bytes` (or
 * never went through pick at all) still gets a live HTTP 413. Two on
 * groq-gpt-oss-120b on 2026-09-08/09 asked for 17124 and 22583 tokens against
 * an observed ceiling of 8000 that had not moved in days — the ceiling was
 * known; nothing before the request had consulted it.
 */
export function oversizedPromptReason(agentEntry, fullPrompt, stateRecord, inheritedLimits, now = Math.floor(Date.now() / 1000)) {
  const estimatedTokens = Math.ceil(Buffer.byteLength(fullPrompt || "", "utf-8") / PROMPT_BYTES_PER_TOKEN);
  const { tpm } = effectiveTokenCeiling(agentEntry, stateRecord, inheritedLimits, now);
  if (tpm == null || tpm >= estimatedTokens) return null;
  return `prompt too large for ${agentEntry.id}: ~${estimatedTokens} estimated tokens exceeds its ${tpm}-token ceiling (pick a different agent, shrink the prompt, or pass --allow-oversized-prompt to send it anyway)`;
}
 ...
 */
function resolveEntryEnv(envMap, { quiet = false } = {}) {
  ...
  for (const [k, v] of Object.entries(envMap)) {
    ...
    if (v.startsWith("@file:")) {
      ...
      try {
        ...
        // `quiet` is for the redaction path, which resolves the same map a
        // second time purely to know what to blank. One unreadable file is one
        // warning, not one per sink.
        if (!quiet) console.error(`dispatch: WARN — could not read ${p} for env ${k}: ${e.message}`);
      }
    } else {
      out[k] = v;
    }
...
export function firstFailureReason(result) {
  ...
  // stripBenignStderrNoise, not just stripAnsi: without it the last line of
  // stderr is often `Warning: Input is not a terminal (fd=0).`, which is what
  // three recorded failures ended up carrying as their reason. The filter
  // already existed for the error preview and simply was not applied here.
  const line = _lastMeaningfulLine(stripBenignStderrNoise(stripAnsi(result?.stderr || "")))
    || _lastMeaningfulLine(stripBenignStderrNoise(stripAnsi(result?.output || "")));
...
/**
 * The environment `redact()` should blank against for THIS entry.
 *
 * Both sinks used to fall back to `process.env`, and that is not the set of
 * secrets the worker was handed. A per-entry `env:` block — literal or
 * `@file:` — is applied to the SUBPROCESS ONLY (see runDispatch's `childEnv`),
 * so a credential injected that way is one the parent never holds, one the value
 * layer therefore cannot see, and one that would survive verbatim in a log if
 * the CLI echoed it back in an error. The pattern layer might catch it by shape;
 * that is a backstop, not a plan.
 *
 * Nothing bundled uses `env:` today, which is why this was never an observed
 * leak and is fixed as a property rather than as an incident.
 *
 * Resolved fresh rather than cached: `runDispatch` resolves the same map for the
 * child on every call, so a cache here would let a rotated `@file:` credential
 * reach the worker while redaction still blanked the old one.
 */
export function redactionEnv(agentEntry, transport) {
  if (!agentEntry || typeof agentEntry !== "object") return process.env;
  const cfg = transport ? getTransportConfig(agentEntry, transport) : null;
  return {
    ...process.env,
    ...resolveEntryEnv(agentEntry.env, { quiet: true }),
    ...resolveEnvFrom(cfg?.env_from, agentEntry),
  };
}
...
// git porcelain statuses that ARE work despite carrying no bytes. Without this
// clause a successful deletion would be re-coded as a failure, which is worse
// than the bug this guard exists to catch.
//
// Matched in EITHER column, not anchored at the start. Porcelain status is `XY`
// — X the index, Y the worktree — so a staged delete is `D ` and an unstaged one
// is ` D`; an anchored test silently missed the second, caught by the test that
// enumerates the statuses rather than by reading this line. `parseGitPorcelain`
// happens to trim, so today only `D` arrives, but the guard must not depend on a
// caller's trimming. D and R carry no other meaning anywhere in the porcelain
// alphabet ( MADRCU?!), so testing for their presence cannot over-match.
const DELETION_OR_RENAME = /[DR]/;

/**
 * Did this run leave anything that counts as work?
 *
 * `files.length > 0` used to stand in for that, and a single 0-BYTE file
 * defeated it — which is the ordinary shape of a failed planner run, not a
 * corner case: `buildAiderArgs` refuses to run without a `--file`, aider
 * touches a declared path that does not exist into existence, and a planner is
 * told to write its plan to exactly such a path. Measured before this guard: a
 * 0-byte file plus an all-429 stdout produced `outcome: success`, and because
 * success resets `consecutive_failures` and writes `state: healthy`, the
 * rate-limited agent was picked again immediately — defeating the cooldown
 * ladder and the account-wide free-tier bucket collapse, since a 429 arriving
 * as exit 0 never marks the bucket.
 *
 * The two listing paths do not agree on fields, and that asymmetry is the trap:
 * a temp workdir is enumerated by `listFiles` → `{path, bytes}`, an external cwd
 * by `listChangedFiles` → `parseGitPorcelain` → `{path, status}` with NO bytes.
 * So bytes are used when present and resolved lazily otherwise, leaving both
 * listing functions' shapes untouched.
 *
 * Fails CLOSED on anything it cannot confirm, and the reason that is safe is
 * NOT that a provider error has already been seen — both callers ask this
 * question first. It is that a `false` return re-codes nothing on its own: it
 * only declines to short-circuit, handing the decision to the pattern or marker
 * check that follows. A failure is therefore never invented from an unreadable
 * file alone; it takes an unconfirmable write AND an error the caller recognises.
 * Keep that order when changing either caller — inverting it would turn this
 * into a general "unreadable means broken" rule, which it must not be.
 *
 * Callers whose `files` entries carry no `bytes` (anything fed from
 * `parseGitPorcelain`) MUST pass `workdir`; with neither, nothing can be
 * confirmed and every entry reads as no work.
 */
export function hasSubstantiveOutput(files, workdir) {
  let root = null;
  try {
    if (workdir) root = fs.realpathSync(workdir);
  } catch { /* an unresolvable workdir leaves only the bytes we were handed */ }

  return (files || []).some((f) => {
    if (DELETION_OR_RENAME.test(f.status ?? "")) return true;
    if (typeof f.bytes === "number") return f.bytes > 0;
    if (!root) return false;
    try {
      // realpath the CANDIDATE, not just the root: a symlink INSIDE the workdir
      // pointing outside it passes a string-prefix test on the unresolved path.
      // realpath also throws ENOENT on a path that was never created and on a
      // dangling symlink — both handled by the catch below as "no content",
      // which is exactly the shape this guard is looking for.
      const abs = fs.realpathSync(path.resolve(root, f.path));
...
/** Declared markers, minus anything that would match everything or nothing. */
export function normalizeFailureMarkers(markers) {
  return (Array.isArray(markers) ? markers : [])
    .filter((m) => typeof m === "string" && m.trim() !== "");
}

// `workdir` is required whenever `files` entries lack `bytes` — see
// hasSubstantiveOutput. Without it such entries cannot be confirmed as work.
export function aiderExitCode(code, isAider, stdout, stderr, files, workdir) {
  if (!isAider || code !== 0) return code;
  if (hasSubstantiveOutput(files, workdir)) return code;
...
/**
 * The same guard for a non-aider CLI, driven by markers the ENTRY declares.
 *
 * Twelve registry entries have a non-aider `edit_exists` (`opencode`, `kiro`,
 * `codex`, the `agy` family), and a rate-limited one of those printing its own
 * error shape and exiting 0 is the same defect wearing a different CLI. What it
 * is NOT is the same vocabulary: a consensus round rejected generalising the
 * litellm pattern — or any repetition heuristic over "provider-error markers" —
 * because neither that term nor "substantive line" can be defined for twelve
 * unrelated CLIs, which makes such a rule unimplementable and untestable.
 *
 * So the vocabulary lives where the command that produces it lives:
 *
 *   transports:
 *     edit_exists:
 *       cmd: "kiro-cli chat …"
 ...
 *
 * An entry that declares none keeps its previous behaviour exactly — no
 * inference, no guessing. aider keeps its built-in pattern above, and that
 * asymmetry is deliberate: the built-in earned its place from observed runs,
 * and a reader who "unifies" the two brings back the guessed vocabulary.
 *
 * Static strings do go stale when a provider rewrites its wording, and this
 * does not pretend otherwise. It makes the fix a DATA edit — the bundled
 * registry, or `agents.local.yaml` per machine — instead of a release. Detecting
 * drift automatically is an explicit non-goal: a heuristic that guesses whether
 * an unknown line is a provider error is the thing that was rejected.
 */
// `workdir` is required whenever `files` entries lack `bytes` — see
// hasSubstantiveOutput. Without it such entries cannot be confirmed as work.
export function declaredMarkerExitCode(code, markers, stdout, stderr, files, workdir) {
  if (code !== 0) return code;
  const declared = normalizeFailureMarkers(markers);
  if (declared.length === 0) return code;
  if (hasSubstantiveOutput(files, workdir)) return code;
  const haystack = `${stdout ?? ""}\n${stderr ?? ""}`;
  return declared.some((m) => haystack.includes(m)) ? 1 : code;
}
...
export function runDispatch(agentEntry, prompt, options = {}, transportKind = "edit_exists") {
  ...
  return new Promise((resolve) => {
    ...
    child.on("close", (code) => {
      ...
      // Structured CLI output, when the entry declares how to read it.
      //
      // Asking a CLI for JSON turns stdout from "the reply" into "an envelope
      // containing the reply", so the answer has to be unwrapped before anything
      // else looks at it — and `emptyRunReason` below is the reason the order
      // matters. `claude --output-format json` returns a perfectly valid,
      // non-empty envelope whose `result` is `""` when the model said nothing.
      // Checking emptiness against raw stdout would score that as work done,
      // which is precisely the false-success class 0.52.0 and 0.52.1 existed to
      // close, one layer further out.
      //
      // parseCliUsage returns null on ANY mismatch, so a wrong or absent spec
      // costs nothing: the answer stays raw stdout and usage stays unknown.
      const cliUsage = parseCliUsage(cleanStdout, agentEntry?.usage_from);
      // An extracted EMPTY string is kept, not discarded. That is the point, not
      // an oversight: an envelope whose `result` is "" means the model produced
      // nothing, and `?? cleanStdout` only fires when there is no extracted text
      // at all. Falling back to the envelope here would restore exactly the
      // false success this unwrapping exists to prevent — a 19 KB envelope
      // reported as work because it is not blank.
      const answerText = cliUsage?.text ?? cleanStdout;
      ...
      const emptyReason = timedOut ? null : emptyRunReason(answerText, cleanStderr, files);
      ...
      const baseExitCode = timedOut
        ? 124
        : emptyRunExitCode(
            // Both marker scans deliberately take cleanStdout, NOT answerText.
            // Emptiness is a question about the ANSWER, so it is asked of the
            // unwrapped text; a provider error is a question about the whole
            // RESPONSE, and an error carried in a field the spec does not select
            // would vanish from an unwrapped view. Raised in review because the
            // intent lived in a commit message instead of here.
            declaredMarkerExitCode(
              aiderExitCode(code, isAiderCommand(parts), cleanStdout, cleanStderr, files, workdir),
              cliTransport?.failure_markers,
              cleanStdout,
              cleanStderr,
              files,
              workdir,
      ...
      resolve({
        ...
        timedOut,
        output: answerText,
        // Present only when the CLI actually reported them. Left absent rather
        // than zeroed: 1984 successful dispatches carried no token data, and a
        // confident 0 is worse than a known gap.
        ...(cliUsage
          ? {
              tokens_in: cliUsage.tokens_in ?? undefined,
              tokens_out: cliUsage.tokens_out ?? undefined,
              ...(cliUsage.cost_usd != null ? { cost_usd: cliUsage.cost_usd } : {}),
              ...(cliUsage.cache_read != null ? { cache_read: cliUsage.cache_read } : {}),
              ...(cliUsage.cache_write != null ? { cache_write: cliUsage.cache_write } : {}),
            }
          : {}),
...
export async function verifyCredential(agentEntry) {
  ...
  const logVerifyFailure = (v, rawBody, extra = {}) => {
    ...
    }, { env: redactionEnv(agentEntry, "generate_new") });
    return v;
  };
  const envName = g.env;
  ...
  try {
    ...
    // A one-token ping comes back with the provider's complete rate-limit header
    // set, which makes `audit` a limits-DISCOVERY pass and not just a liveness
    // one: a fresh install learns every ceiling it has for the price of a probe
    // it was already running, instead of learning them one HTTP 413 at a time.
    const responseHeaders = Object.fromEntries(
      [...resp.headers].filter(([k]) => !/^set-cookie$/i.test(k)),
    );
    if (resp.ok) return { ok: true, latencyMs, responseHeaders };
...
/**
 * A provider error carried inside a 200 envelope.
 *
 * OpenAI-compatible endpoints are supposed to signal failure with a status
 * code, and several do not: a 200 arrives with `{"error": {...}}` and no
 * `choices`. That was already treated as a failure here, but for the wrong
 * reason — an empty `content` was reported as `empty generated output`, and
 * since the classifier reads only `stderr + output`, the envelope's own words
 * never reached it. So a 429 delivered this way was scored as a generic
 * transient fault: the agent got the failure ladder instead of a quota
 * cooldown, and the account-wide free-tier bucket was never marked. Same class
 * as a 0-byte file passing for work, one layer up.
 *
 * Structured only, deliberately. The sibling case — a 200 whose CONTENT is a
 * model politely saying it is rate limited — is not a defect and is not treated
 * as one: prose is the model's answer, and inferring failure from it is the
 * content-guessing a four-round consensus rejected for the CLI guard.
 *
...
export function envelopeError(data) {
  ...
  if (typeof err === "string") {
    return err.trim() === "" ? null : { message: err, status: null };
  }
  if (typeof err !== "object") return null;
  const message = [err.message, err.type, err.code]
    .filter((v) => typeof v === "string" && v.trim() !== "")
    .join(" — ");
  // `code` is a number for some providers and a slug ("rate_limit_exceeded")
  // for others; only a number can stand in for an HTTP status.
  const numeric = typeof err.code === "number" ? err.code
    : typeof err.status === "number" ? err.status
    : /^\d{3}$/.test(String(err.code ?? "")) ? Number(err.code)
    : null;
  return { message: message || "provider returned an error object with no message", status: numeric };
}
...
export async function runGenerate(agentEntry, prompt, options = {}) {
  ...
  try {
    ...
    // Hoisted out of the individual returns below, and attached to ALL of them
    // including success.
    //
    // It used to be built inside the two error branches only, on the reasonable-
    // sounding grounds that rate-limit headers matter when you have been rate
    // limited. That is backwards: by the time a 429 arrives the information is
    // an autopsy, and the same headers on the preceding SUCCESS were a warning
    // that could have re-routed the next call. Every provider in the pool sends
    // `x-ratelimit-remaining-*` on a 200. Discarding them there is what made the
    // whole system reactive — it could only ever discover a limit by breaking on
    // it. Cookies stay out: both useless here and credential-bearing.
    const responseHeaders = Object.fromEntries(
      [...resp.headers].filter(([k]) => !/^set-cookie$/i.test(k)),
    );
    if (!resp.ok) {
    ...
    // Checked BEFORE content: a provider that puts its error in a 200 envelope
    // has already told us the run failed, and its own words are what the
    // classifier needs in order to score a 429 as a quota event rather than a
    // generic fault. `status` is set from the envelope when it carries a numeric
    // code, so the reported reason reads `HTTP 429` exactly as it would have on
    // a correctly-coded response.
    const envErr = envelopeError(data);
    if (envErr) {
      return {
        output: "",
        stderr: envErr.status ? `HTTP ${envErr.status} — ${envErr.message}` : envErr.message,
        exitCode: 1,
        durationMs,
        workdir,
        files: [],
        ...
        status: envErr.status ?? resp.status,
        request: requestDescriptor,
        responseBody: bodyText,
        responseHeaders,
      };
 ...
 *
 * Always appends one JSONL row to the dispatch log regardless of outcome, so
 * get_stats can aggregate. The write is best-effort and never fails a dispatch;
 * where the file lives, how it is bounded and how it is read back are all in
 * lib/dispatch-log.js.
 */
...
export async function runAny(agentEntry, prompt, options = {}) {
  // A transport refusal is a PRE-DISPATCH refusal: nothing is spawned, there is
  // no exit code, and before this it left no trace in the failure log at all —
  // the same class the sidecar log was built to capture (unknown agent, disabled
  // agent, --require-base mismatch), on the one path that was missed. Recorded
  // here rather than at each surface so the CLI and the MCP server cannot drift.
  let transport;
  try {
    transport = selectTransport(agentEntry, options);
  } catch (e) {
    e.preDispatchRefusal = true;
    recordFailure({
      stage: "precheck",
      outcome: "refused",
      reason: e.message,
      ...
      requested_transport: options?.transport ?? null,
      declared_transports: Object.keys(agentEntry?.transports || {}),
    }, { env: redactionEnv(agentEntry, options?.transport) });
    throw e;
  }
  let result;

  // A `deprecated` note is a free-text advisory on the registry entry, printed
  // once per dispatch. It never blocks or alters the transport — an entry
  // stays fully functional right up until it is removed from agents.yaml.
  if (agentEntry.deprecated) {
    console.error(`dispatch: WARNING — ${agentEntry.id} is deprecated: ${agentEntry.deprecated}`);
  }
  ...
  // A second PRE-DISPATCH refusal, same shape as the transport one above:
  // `pick --prompt-bytes` has refused to SEAT an oversized prompt since the
  // registry gained token_limits (#51), but a direct `dispatch <agent-id>` —
  // by definition skipping pick — was never checked against the same ceiling.
  // `options.allowOversizedPrompt` is the deliberate override, mirroring
  // --require-base's opt-in-only shape: silence here is "unknown", not "fits".
  if (!options.allowOversizedPrompt) {
    const reason = oversizedPromptReason(
      agentEntry,
      fullPrompt,
      readState()[agentEntry.id],
      options.inheritedTokenLimits,
    );
    if (reason) {
      ...
      }, { env: redactionEnv(agentEntry, transport) });
      throw e;
    }
  }
  ...
  // Resolved once and shared by both sinks: the entry's own env blocks are
  // applied to the subprocess only, so `process.env` is not the set of secrets
  // this dispatch actually handed out. See redactionEnv.
  const dispatchRedactionEnv = redactionEnv(agentEntry, transport);
  ...
  appendDispatchRow({
    ...
    // Recorded separately, never folded into tokens_in. They are real tokens
    // and they bill at different rates (write ~1.25x, read ~0.1x), so a single
    // summed field could serve either the count or the cost and would be wrong
    // for the other one.
    cache_read: result.cache_read ?? null,
    cache_write: result.cache_write ?? null,
    prompt_bytes: Buffer.byteLength(fullPrompt || "", "utf-8"),
    file_context_bytes: fileContext ? Buffer.byteLength(fileContext, "utf-8") : 0,
    http_status: result.status ?? null,
...
export function getStats(sinceIso) {
  // The log holds the last 30 days by construction (see dispatch-log.js), so a
  // window longer than that is answered with whatever is left rather than an
  // error. With no log at all this yields the same empty shape the old
  // existsSync early-return did.
  const rows = readDispatchRows();
  ...
  for (const r of filtered) {
    const a = (by_agent[r.agent_id] ??= {
      count: 0, tokens_in: 0, tokens_out: 0, cache_read: 0, cache_write: 0, duration_ms: 0,
      outcomes: {}, transports: {},
    ...
    const t = (by_transport[r.transport] ??= { count: 0, tokens_in: 0, tokens_out: 0, cache_read: 0, cache_write: 0 });
    t.count++; t.tokens_in += r.tokens_in || 0; t.tokens_out += r.tokens_out || 0;
    t.cache_read += r.cache_read || 0; t.cache_write += r.cache_write || 0;
...
// Probes that must not outlive us.
//
// The 90s SIGKILL below lives in THIS process, so it can only fire while this
// process is alive. When the parent dies first — a harness timeout, a Ctrl-C, a
// supervisor's SIGTERM — the timer dies with it and the CLI is reparented to
// PID 1, where nothing will ever end it. Found in the wild: three stray probes,
// one 5h20m old, two still holding ESTABLISHED sockets to a provider gateway.
//
// So the probe is spawned DETACHED, as its own process-group leader, and every
// exit path this process can still observe kills that whole group. Detaching is
// what makes the group kill reach a CLI's own children (a `cmd | cmd` or
// `cmd && cmd` transport leaves bash alive with children under it, and killing
// bash alone would strand them); it also means the probe no longer receives the
// terminal's Ctrl-C by inheritance, which is precisely why the handlers below
// are not optional — detaching without them would make orphans MORE likely.
//
// This module is imported by the long-running server, so the handlers are scoped
// to the window where a probe is actually in flight: installed with the first
// live probe, removed with the last. A library may borrow the process's signals
// for the length of an operation; it may not keep them afterwards.
//
// A SIGKILL'd parent remains unfixable from inside the process, by definition.
const PROBE_SIGNALS = ["SIGINT", "SIGTERM", "SIGHUP"];
const liveProbes = new Set();
let probeHandlers = null;

function killProbeGroup(child) {
  // The negative pid addresses the group; fall back to the child alone if it
  // never became a leader (spawn failure) or the group is already gone.
  try { process.kill(-child.pid, "SIGKILL"); return; } catch {}
  try { child.kill("SIGKILL"); } catch {}
}

function reapLiveProbes() {
  for (const child of liveProbes) killProbeGroup(child);
  liveProbes.clear();
}

function trackProbe(child) {
  liveProbes.add(child);
  // Once, not once per probe: `audit` probes dozens of entries, and a listener
  // apiece would trip MaxListenersExceeded and leak handlers of its own.
  if (probeHandlers) return;
  const onExit = () => reapLiveProbes();
  const signals = new Map();
  for (const sig of PROBE_SIGNALS) {
    // Remove only OUR listener before re-raising, never every listener for the
    // signal: wiping the process's handlers would silently cancel a consumer's
    // own shutdown. Dropping just ours avoids the recursion and leaves anyone
    // else's decision intact — and with no other listener the re-raise
    // terminates as normal, so the status still reads as "killed by <sig>".
    const onSignal = () => {
      reapLiveProbes();
      process.removeListener(sig, onSignal);
      process.kill(process.pid, sig);
    };
    signals.set(sig, onSignal);
    process.on(sig, onSignal);
  }
  process.on("exit", onExit);
  probeHandlers = { onExit, signals };
}

function untrackProbe(child) {
  liveProbes.delete(child);
  if (liveProbes.size > 0 || !probeHandlers) return;
  process.removeListener("exit", probeHandlers.onExit);
  for (const [sig, handler] of probeHandlers.signals) process.removeListener(sig, handler);
  probeHandlers = null;
}
...
export async function auditCliEntry(entry) {
  ...
  return new Promise((rawResolve) => {
    const child = spawn("bash", ["-c", shellCommand], {
      stdio: ["ignore", "pipe", "pipe"],
      env: process.env,
      // Own process group, so the kills below reach the CLI's children too.
      detached: true,
    });
    trackProbe(child);
    let out = "", err = "";
    ...
    const resolve = (v) => {
      if (!v.ok) {
        ...
        }, { config: failureLog, env: redactionEnv(entry, "edit_exists") });
      }
      rawResolve(v);
    };
    ...
    let timedOut = false;
    // A spawn that never starts still has to be let go of: a stale entry would
    // pin the handlers in place for the life of the process and leave a dead pid
    // to be killed on the way out.
    child.on("error", () => untrackProbe(child));
    const timer = setTimeout(() => { timedOut = true; killProbeGroup(child); }, 90000);
    child.on("close", (code) => {
      clearTimeout(timer);
      untrackProbe(child);
      const latencyMs = Date.now() - start;
...
export async function probeReadOnlyNonWriting(entry) {
  ...
  const fail = (v, raw = {}) => {
    ...
    }, { env: redactionEnv(entry, "read_only") });
    return v;
  };
