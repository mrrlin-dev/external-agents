...
let probeReaperInstalled = false;
...
function installProbeReaper() {
  // Once per process: `audit` probes dozens of entries and a listener per probe
  // would trip MaxListenersExceeded and leak its own handlers.
  if (probeReaperInstalled) return;
  probeReaperInstalled = true;
  process.on("exit", reapLiveProbes);
  for (const sig of ["SIGINT", "SIGTERM", "SIGHUP"]) {
    process.on(sig, () => {
      reapLiveProbes();
      // Re-raise so the exit status still reads as "killed by <sig>". Adding a
      // listener suppressed the default termination; restoring it is our job.
