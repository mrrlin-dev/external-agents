...
export function resolveFileContext(files, basedir) {
...
export function runDispatch(agentEntry, prompt, options = {}) {
  ...
  if (isAiderCommand(parts)) {
    args = [...parts.slice(1), ...effortParts, "--message", prompt, ...AIDER_HEADLESS_FLAGS];
  } else {
    args = [...parts.slice(1), ...effortParts, prompt];
  }
...
export async function runAny(agentEntry, prompt, options = {}) {
  ...
  // Resolve file context and prepend to prompt. Require cwd when files are
  // given — without it, paths resolve against the MCP server's process.cwd()
  // which is almost certainly wrong and will fail containment silently.
  ...
  } else if (getTransportConfig(agentEntry, "generate_new")) {
    transport = "generate_new";
    result = await runGenerate(agentEntry, fullPrompt, options);
  } else if (getTransportConfig(agentEntry, "edit_exists")) {
    transport = "edit_exists";
    result = await runDispatch(agentEntry, fullPrompt, options);
  } else {
