...
export function sharedQuotaBucketIds(entry, agents = []) {
  ...
  // (b) `quota_scope: shared` — the registry saying so in as many words.
  //
  // The field has been in the registry all along, on 27 entries, and was read by
  // ZERO lines of code; the rule above was hardcoded next to it instead. So the
  // pattern it exists to describe kept costing rounds everywhere except
  // openrouter: groq-gpt-oss-120b, -20b and -qwen3.6-27b are three entries on ONE
  // key sharing one 8000-TPM window, and exhausting the first left the other two
  // looking healthy to be picked in turn and rediscover the same cap.
  //
  // Matched on the EXACT provider slug, not the family, which is the opposite of
  // (a) and deliberate: `shared` means "shared with the other models behind this
  // key", and groq2 is a different key with its own allowance. Collapsing by
  // family here would take a working key down with an exhausted one.
  if (entry?.quota_scope === "shared" && entry?.provider) {
    for (const a of agents) {
      if (a.id === entry.id) continue;
      if (a.quota_scope === "shared" && a.provider === entry.provider) ids.add(a.id);
    }
  }
...
export function withObservations({ base, prev, result, ok, now }) {
  ...
  // Carried EXPLICITLY, never by spreading `prev` over `base`.
  ...
  const carried = {
    ...(prev?.observed_limits ? { observed_limits: prev.observed_limits } : {}),
    ...(prev?.observed_budget ? { observed_budget: prev.observed_budget } : {}),
  };
  return {
    ...mergeObserved({ ...base, ...carried }, observed),
