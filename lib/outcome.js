import { observedFromResponse, mergeObserved, raisedByAcceptedPrompt, tokenAxisCooldownFloor } from "./budget.js";
...
export function withObservations({ base, prev, result, ok, now }) {
  ...
  const merged = mergeObserved(base, observed);
  // A prompt the provider ACCEPTED is proof the input ceiling is at least that
  // large. Applied against the record we are about to write, and only upward, so
  // an input axis learned from a rejection can be corrected without waiting for
  // another one. `prev` is deliberately not consulted: the same staleness that
  // bars it from carry-forward bars it here.
  const raised = ok ? raisedByAcceptedPrompt(merged, Number(result?.tokens_in), now, result?.transport) : null;
  return {
    ...merged,
    ...(raised || {}),
