...
test('parseStatedLimit does not lose the axis when a separator orphans the limit', () => {
  // A clause boundary between the marker and its number must not launder an
  // output ceiling into an untagged one — that is the original corruption with
  // a line break in it. A clause that says nothing defers to the nearest marker
  // in the text; a clause that names its own axis never does.
  assert.equal(parseStatedLimit('on output tokens per minute (OTPM):\nLimit 1000, Requested 1434'), null);
  assert.equal(parseStatedLimit('Exceeded OTPM. Limit 1000, Requested 1434'), null);
  assert.deepEqual(
    parseStatedLimit('on tokens per minute (TPM):\nLimit 8000, Requested 10098'),
    { limit: 8000, requested: 10098 },
  );
});
...
test('parseStatedLimit rejects the malformed shapes rather than inventing a ceiling', () => {
  // Consensus round 3 asked for these explicitly. A ceiling is only ever read
  // from a complete statement: a limit with no Requested count is not one, and
  // a zero or non-numeric limit would gate every prompt off the seat forever.
  assert.equal(parseStatedLimit('Limit 1000'), null);
  assert.equal(parseStatedLimit('Limit abc, Requested xyz'), null);
  assert.equal(parseStatedLimit('on tokens per minute (TPM): Limit 0, Requested 5'), null);
  // Whitespace around the comma is the provider's business, not ours.
  assert.deepEqual(
    parseStatedLimit('on tokens per minute (TPM): Limit 1000 , Requested 2000'),
    { limit: 1000, requested: 2000 },
  );
  // And the same shape on the output axis is still refused.
  assert.equal(
    parseStatedLimit('on output tokens per minute (OTPM): Limit 1000 , Requested 2000'),
    null,
  );
});

test('parseStatedLimit is unbothered by the dots and slashes inside a model name', () => {
  // Round 3 claimed a model name would break clause splitting. It does not: a
  // period only ends a clause when whitespace follows it.
  const body = 'Request too large for model `qwen/qwen3.6-27b` in organization `org_x` '
    + 'on input tokens per minute (ITPM): Limit 7000, Requested 8128';
  assert.deepEqual(parseStatedLimit(body), { limit: 7000, requested: 8128 });
  const otpm = 'Request too large for model `qwen/qwen3.6-27b` '
    + 'on output tokens per minute (OTPM): Limit 1000, Requested 1434';
  assert.equal(parseStatedLimit(otpm), null);
});
...
test('effectiveTokenCeiling: observation beats declaration in both directions', () => {
  ...
  assert.deepEqual(
    effectiveTokenCeiling(
      { id: 'a' },
      { observed_limits: { tpm: 5000, axis_seen_at: { tpm: now }, seen_at: now } },
      null,
      now,
    ),
  ...
  assert.deepEqual(
    effectiveTokenCeiling(
      ...
      { observed_limits: { tpm: 12_000, axis_seen_at: { tpm: now }, seen_at: now } },
      null,
      now,
    ),
...
// Per-axis ceilings.
//
// groq's `x-ratelimit-limit-tokens: 8000` is the COMBINED budget. The thing that
// actually bounds a PROMPT on qwen3.6-27b is an input budget that appears in no
// header and only ever in error prose: `ITPM: Limit 7000`. Because the record
// held one axis-less `tpm`, any later headers-only response — a successful audit
// probe is exactly that — overwrote a body-learned 7000 with the header's 8000,
// and pick then seated prompts groq rejects. Three did, in one afternoon.
// ---------------------------------------------------------------------------

test('parseStatedLimits reports every axis a body states, separately', () => {
  const body = 'on input tokens per minute (ITPM): Limit 7000, Requested 8128; '
    + 'on output tokens per minute (OTPM): Limit 1000, Requested 1434';
  assert.deepEqual(parseStatedLimits(body), {
    input: { limit: 7000, requested: 8128 },
    output: { limit: 1000, requested: 1434 },
  });
  assert.deepEqual(
    parseStatedLimits('on tokens per minute (TPM): Limit 8000, Requested 10098'),
    { combined: { limit: 8000, requested: 10098 } },
  );
  assert.deepEqual(parseStatedLimits('nothing here'), {});
});

test('parseStatedLimit still answers with the prompt-bounding limit', () => {
  // The old contract, unchanged: callers that want one number get the one that
  // bounds a prompt, and never the output allowance.
  assert.deepEqual(
    parseStatedLimit('on input tokens per minute (ITPM): Limit 7000, Requested 8128'),
    { limit: 7000, requested: 8128 },
  );
  assert.equal(
    parseStatedLimit('on output tokens per minute (OTPM): Limit 1000, Requested 1434'),
    null,
  );
});

test('observedFromResponse files each axis under its own name and timestamp', () => {
  const o = observedFromResponse({
    bodyText: 'on input tokens per minute (ITPM): Limit 7000, Requested 8128',
    now: 1000,
  });
  assert.equal(o.observed_limits.itpm, 7000);
  assert.equal(o.observed_limits.axis_seen_at.itpm, 1000);
  // An input ceiling is NOT a combined one and must not be written as though it were.
  assert.equal(o.observed_limits.tpm, undefined);

  const out = observedFromResponse({
    bodyText: 'on output tokens per minute (OTPM): Limit 1000, Requested 1434',
    now: 1000,
  });
  assert.equal(out.observed_limits.otpm, 1000);
  assert.equal(out.observed_limits.tpm, undefined, 'an output allowance is not a prompt ceiling');
});

test('a headers-only observation cannot overwrite a body-learned input ceiling', () => {
  // The exact sequence that broke it: learn ITPM 7000 from a 413, then run a
  // successful audit whose response carries only combined headers.
  const learned = observedFromResponse({
    bodyText: 'on input tokens per minute (ITPM): Limit 7000, Requested 8128',
    now: 1000,
  });
  const later = observedFromResponse({
    headers: { 'x-ratelimit-limit-tokens': '8000' },
    now: 2000,
  });
  const merged = mergeObserved(learned, later);
  assert.equal(merged.observed_limits.itpm, 7000, 'the input axis must survive');
  assert.equal(merged.observed_limits.tpm, 8000, 'and the combined axis is still learned');
});

test('effectiveTokenCeiling bounds a prompt by the input axis, not the combined one', () => {
  const record = {
    observed_limits: {
      tpm: 8000, itpm: 7000, otpm: 1000,
      axis_seen_at: { tpm: 1000, itpm: 1000, otpm: 1000 },
      source: 'error_body', seen_at: 1000,
    },
  };
  // min(itpm, tpm - otpm) = min(7000, 7000) = 7000. Both routes agree here, and
  // the three real rejections requested 7040, 7592 and 8005 — all above it.
  assert.equal(effectiveTokenCeiling(null, record, null, 1000).tpm, 7000);
});

test('effectiveTokenCeiling reserves the output allowance out of a combined budget', () => {
  // The combined budget must hold prompt AND output. Measured: for comparable
  // prompts groq counts more against TPM than against ITPM — a 25121-byte prompt
  // drew `TPM requested=8733` while a larger 26770-byte one drew `ITPM 7040`.
  const record = {
    observed_limits: {
      tpm: 8000, otpm: 2000,
      axis_seen_at: { tpm: 1000, otpm: 1000 },
      source: 'headers', seen_at: 1000,
    },
  };
  assert.equal(effectiveTokenCeiling(null, record, null, 1000).tpm, 6000);
});

test('effectiveTokenCeiling reads the declared itpm/otpm the schema already has', () => {
  // agents.yaml has carried `token_limits: { itpm, otpm }` all along — on
  // claude-opus-5 among others — and no code has ever read them.
  const entry = { token_limits: { tpm: 8000, itpm: 7000, otpm: 1000 } };
  assert.deepEqual(effectiveTokenCeiling(entry, null, null, 1000), { tpm: 7000, source: 'declared' });
});

test('a legacy axis-less record is discarded rather than guessed at', () => {
  // Its `tpm` might be combined, input, or — for the two seats this whole change
  // came from — an OTPM value misparsed as a ceiling. `source` cannot tell them
  // apart, so the honest move is to forget it and re-learn.
  const legacy = { observed_limits: { tpm: 1000, source: 'error_body', seen_at: 1000 } };
  const entry = { token_limits: { tpm: 8000 } };
  assert.deepEqual(
    effectiveTokenCeiling(entry, legacy, null, 1000),
    { tpm: 8000, source: 'declared' },
    'fall through to the declared value instead of trusting an axis-less number',
  );
});


test('an accepted prompt raises a stale input ceiling, and never lowers one', () => {
  // The input axis is taught only by rejection, so without this it is a one-way
  // ratchet: once the gate is low enough the 413s stop, and with them the only
  // evidence that could raise it again.
  const rec = {
    observed_limits: {
      itpm: 7000, tpm: 8000,
      axis_seen_at: { itpm: 1000, tpm: 1000 },
      source: 'error_body', seen_at: 1000,
    },
  };
  const raised = raisedByAcceptedPrompt(rec, 7600, 5000, 'generate_new');
  assert.equal(raised.observed_limits.itpm, 7600, 'the provider accepted 7600, so 7000 was stale');
  assert.equal(raised.observed_limits.axis_seen_at.itpm, 5000);
  assert.equal(raised.observed_limits.tpm, 8000, 'other axes are untouched');

  // A smaller accepted prompt proves nothing about where the ceiling stops.
  assert.equal(raisedByAcceptedPrompt(rec, 100, 5000, 'generate_new'), null);
  assert.equal(raisedByAcceptedPrompt(rec, 0, 5000, 'generate_new'), null);
  assert.equal(raisedByAcceptedPrompt(rec, NaN, 5000, 'generate_new'), null);
  // A legacy record has no axis to raise.
  assert.equal(raisedByAcceptedPrompt({ observed_limits: { tpm: 8000 } }, 9000, 5000, 'generate_new'), null);

  // A CLI transport's `tokens_in` is not the prompt. `claude --print` reported
  // input_tokens 10 beside cache_read 21675 on a measured run, so trusting it
  // here would raise a ceiling on a number that means something else entirely.
  assert.equal(raisedByAcceptedPrompt(rec, 7600, 5000, 'edit_exists'), null);
  assert.equal(raisedByAcceptedPrompt(rec, 7600, 5000, null), null);
});
