...
test('an accepted prompt raises a stale input ceiling, and never lowers one', () => {
  ...
  const raised = raisedByAcceptedPrompt(rec, 7600, 5000);
  ...
  assert.equal(raisedByAcceptedPrompt(rec, 100, 5000), null);
  assert.equal(raisedByAcceptedPrompt(rec, 0, 5000), null);
  assert.equal(raisedByAcceptedPrompt(rec, NaN, 5000), null);
  // A legacy record has no axis to raise.
  assert.equal(raisedByAcceptedPrompt({ observed_limits: { tpm: 8000 } }, 9000, 5000), null);
});
