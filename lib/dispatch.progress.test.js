...
test("buildAiderArgs writes .aiderignore with only * when no files are attached", () => {
  // Even with no attached files, we write the ignore file containing just *.
  // This converts a silent "aider guessed which files to touch" into an
  // explicit empty run (the model gets no files and must ask).
  const repoDir = fs.mkdtempSync(path.join(os.tmpdir(), "ea-aider-nofiles-"));
  fs.mkdirSync(path.join(repoDir, ".git"), { recursive: true });
  const workdir = path.join(repoDir, "sub");
  fs.mkdirSync(workdir, { recursive: true });

  try {
    const args = buildAiderArgs(["aider"], [], "prompt", workdir, { files: [] });

    const ignoreIdx = args.indexOf("--aiderignore");
    assert.ok(ignoreIdx > -1, "should pass --aiderignore even with no files");
    const ignorePath = args[ignoreIdx + 1];
    const content = fs.readFileSync(ignorePath, "utf-8");
    assert.equal(content.trim(), "*", "ignore file should contain only * when no files attached");
  } finally {
    fs.rmSync(repoDir, { recursive: true, force: true });
  }
});
