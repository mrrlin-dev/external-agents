...
test("runDispatch removes parent signal handlers after completion", async () => {
  ...
  try {
    const result = await runDispatch(
      ...
      "ignored",
      // 15s, not 1s. This test is about signal-handler CLEANUP, and with a
      // one-second budget it was also quietly asserting that a cold `node`
      // spawn finishes inside a second — which it does not when the full suite
      // runs its files in parallel. It failed roughly one run in six, always
      // the same way: exitCode 124 (timed out) at duration_ms ~1450, so the
      // cleanup assertions below were never even reached. A generous timeout
      // costs nothing here because the happy path returns in milliseconds; only
      // a real hang would ever spend it.
      { timeoutMs: 15000 },
    );
