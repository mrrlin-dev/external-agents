...
test("selectTransport routes an explicitly declared read_only via generate_new", () => {
  const declared = {
    id: "generate-only",
    transports: {
      generate_new: both.transports.generate_new,
      read_only: { via: "generate_new", verified: "by_construction" },
    },
  };
  assert.equal(selectTransport(declared, { transport: "read_only" }), "generate_new");
});

test("selectTransport no longer treats a bare generate_new as read_only by implication", () => {
  // The fallback was real (an HTTP call cannot write) but it made coverage
  // unauditable — an entry nobody had considered looked identical to one
  // deliberately cleared for read-only use. Declarations are explicit now.
  const generateOnly = { id: "generate-only", transports: { generate_new: both.transports.generate_new } };
  assert.throws(
    () => selectTransport(generateOnly, { transport: "read_only" }),
    /does not declare it/,
  );
});
