...
import { pickAgents, explainEmptyPick, formatEmptyPick } from './pick.js';
...
test('pick refuses a seat whose OBSERVED ceiling cannot hold the prompt', () => {
  ...
  const state = {
    small: { state: 'healthy', observed_limits: { tpm: 5000, axis_seen_at: { tpm: now }, seen_at: now } },
    big: { state: 'healthy', observed_limits: { tpm: 200_000, axis_seen_at: { tpm: now }, seen_at: now } },
  };
...
test('explainEmptyPick attributes each entry to the FIRST filter that rejected it', () => {
  ...
  assert.equal(explain.reasons['prompt too large (needs 10000 tokens)'], 1);
