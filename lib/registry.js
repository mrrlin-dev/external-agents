...
import { providerFamily } from "./pick.js";
...
// `add_provider_key` clones a base entry into a numbered sibling by spreading
// its fields at CLONE time (see ui.js's /api/add_provider_key), and that clone
// is then a permanent, independent entry in agents.local.yaml — nothing
// revisits it afterward. If the base later gets `enabled: false` (a provider
// drops free-tier access, a model gets retired), every clone made BEFORE that
// change keeps whatever `enabled` it was cloned with — undefined, i.e. "on" —
// forever. That silently reopened the kill switch for gemini-3.1-pro-preview's
// google3..google8 keys after the base was disabled for exactly this model:
// they were cloned before the disablement and never got `enabled: false`
// themselves, so pickAgents kept seating them.
//
// Re-derive each numbered clone's `enabled` from its base by (provider family,
// model) identity on every load, so a later fix to the base reaches keys that
// existed before the fix. Only fills in the gap — a clone that already has its
// own explicit `enabled` (e.g. state.json's per-key override, which lives on a
// separate layer entirely) is left alone.
function syncClonesWithBase(registry) {
  const baseByModel = new Map();
  for (const agent of registry.agents) {
    if (!/\d+$/.test(agent.provider)) {
      baseByModel.set(`${providerFamily(agent.provider)}::${agent.model}`, agent);
    }
  }
  for (const agent of registry.agents) {
    if (!/\d+$/.test(agent.provider)) continue;
    const base = baseByModel.get(`${providerFamily(agent.provider)}::${agent.model}`);
    if (base?.enabled === false && agent.enabled === undefined) {
      agent.enabled = false;
    }
  }
  return registry;
...
export function loadRegistry(yamlPath) {
  ...
  return syncClonesWithBase(registry);
}
...
export function nextProviderSlot(registry, baseProvider) {
  ...
  // Floor of 2: the base slug IS the first key, so the next one is the second.
  // Matters now that `google2` is no longer bundled — on a fresh install the
  // only match is bare `google` (suffix 0), and a naive max+1 would name the
  // operator's second key `google1`, contradicting both the base slug and the
  // `GEMINI_API_KEY_2` env var derived alongside it.
  return Math.max(maxSuffix + 1, 2);
}
...
export async function withLocalOverlayLock(mutatorFn) {
  ...
  // The state directory may not exist yet: on a machine where nothing has
  // written state, `add-model` was the first thing to touch it and died with
  // ENOENT on the LOCK file — before the overlay it was about to create.
