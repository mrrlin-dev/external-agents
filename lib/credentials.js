...
// of truth now.
const injectedByUs = new Set();
export function bootEnv() {
  try {
    const persisted = loadKeysFile();
    for (const [k, v] of Object.entries(persisted)) {
      if (!process.env[k]) { process.env[k] = v; injectedByUs.add(k); }
    }
  } catch (e) {
...
// Re-read keys.env mid-process. A long-lived process (the UI, the MCP server)
// otherwise keeps whatever env it booted with, so a key added afterwards from
// another shell — `external-agents set-credential DEEPSEEK_API_KEY` — leaves
// the UI still reporting "env var DEEPSEEK_API_KEY not set" for an agent whose
// key is sitting in keys.env. Only vars that are unset, or that bootEnv/this
// function injected themselves, are touched: a value the operator exported in
// their own shell still wins, exactly as at boot.
export function refreshEnv() {
  try {
    const persisted = loadKeysFile();
    for (const [k, v] of Object.entries(persisted)) {
      if (process.env[k] !== undefined && !injectedByUs.has(k)) continue;
      process.env[k] = v;
      injectedByUs.add(k);
    }
  } catch (e) {
    console.error(`external-agents: WARN — refreshEnv failed: ${e.message}`);
  }
}

/**
 * A path with the operator's home directory folded back to `~`.
 *
 * Not cosmetics. Every path this tool prints starts with someone's home
 * directory, and a home directory on a work machine is usually their full name:
 * this repo shipped `docs/03-saved.png` to npm for several releases showing
 * `/Users/<first>.<last>/.local/state/...` in a dashboard confirmation, because
 * nobody looks at a screenshot the way they look at a log line. `~/.local/state/
 * external-agents/keys.env` says exactly as much to the person reading it and
 * nothing at all to anyone they paste it to.
 *
 * Only a leading home directory is folded, and only on an exact segment
 * boundary — `/Users/alice-backup` must not become `~-backup`.
 */
