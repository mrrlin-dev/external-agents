 ...
 *            cost_usd: number|null, cache_read: number|null,
 *            cache_write: number|null}|null}
...
export function parseCliUsage(stdout, spec) {
  ...
  return {
    ...
    // Both cache tiers, because `input_tokens` is NOT the input. Measured on a
    // real `claude --print --output-format json` run: input_tokens 10,
    // cache_creation_input_tokens 16104, cache_read_input_tokens 21675 — the
    // field named "input" was 0.03% of the 37,789 tokens actually processed.
    // Recording only the first is not a rounding error, it is the wrong number.
    cache_read: num(at(node, spec.cache_read)),
    cache_write: num(at(node, spec.cache_write)),
  };
}
