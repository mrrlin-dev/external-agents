...
test('reads agy\'s print-timeout envelope: zero usage and empty response, not a mismatch', () => {
  // Captured live 2026-09-10: when agy's own --print-timeout fires mid-turn,
  // --output-format json still returns a well-formed envelope — status:
  // "ERROR", response: "", usage all zero. num() treats 0 as a real number
  // (not "absent"), so this must NOT parse as a mismatch (which would fall
  // back to raw stdout as the answer) — it must extract text:"" and
  // tokens 0/0, so the caller's answerText is genuinely empty and the
  // existing emptyRunReason/failure_markers guards see the true shape of the
  // run instead of a JSON blob that merely LOOKS non-blank.
  const AGY_PRINT_TIMEOUT_OUT = JSON.stringify({
    conversation_id: 'c1408cb3-a6fa-4508-a136-afa41470a192',
    status: 'ERROR',
    response: '',
    error: 'The stream was interrupted. Please continue the task you were working on.',
    duration_seconds: 12.823739,
    num_turns: 1,
    usage: { input_tokens: 0, output_tokens: 0, thinking_tokens: 0, cache_read_tokens: 0, total_tokens: 0 },
  });
  const u = parseCliUsage(AGY_PRINT_TIMEOUT_OUT, AGY_SPEC);
  assert.ok(u, 'a well-formed envelope with all-zero usage must still parse, not fall back to raw stdout');
  assert.equal(u.text, '');
  assert.equal(u.tokens_in, 0);
  assert.equal(u.tokens_out, 0);
});
...
test('jsonl reads a complete stream', () => {
...
test('jsonl refuses a truncated stream instead of passing an intermediate event as totals', () => {
  // Tolerating a cut tail sounded generous and was wrong: the selection step
  // takes the last matching event as the TOTALS, so a stream cut mid-write would
  // hand back an intermediate count as if the run had finished there. A wrong
  // number is worse than a gap, because a gap is visible.
  const spec = { kind: 'jsonl', event: 'turn.completed', text: 'text', tokens_in: 'usage.in', tokens_out: 'usage.out' };
  const truncated = [
    '{"type":"turn.completed","text":"partial","usage":{"in":5,"out":5}}',
    '{"type":"turn.compl',
  ].join('\n');
  assert.equal(parseCliUsage(truncated, spec), null, 'broken final line → no totals to trust');

  // A banner BEFORE the stream is ordinary and still tolerated.
  const withBanner = [
    'codex v1.2.3 starting…',
    '{"type":"turn.started"}',
    '{"type":"turn.completed","text":"done","usage":{"in":7,"out":9}}',
  ].join('\n');
  const u = parseCliUsage(withBanner, spec);
  assert.equal(u.text, 'done');
  assert.equal(u.tokens_in, 7);
});

test('a dunder path segment does not "line up"', () => {
  // obj["__proto__"] returns the prototype, which is truthy — so without a guard
  // a careless registry path would satisfy the lookup and contradict this
  // module's promise. Nothing was exploitable (num() killed it), the promise was
  // just untrue.
  for (const bad of ['__proto__', 'constructor', 'prototype', 'usage.__proto__']) {
    const spec = { kind: 'json', event: 'result', tokens_in: bad, tokens_out: bad };
    assert.equal(parseCliUsage(CLAUDE_OUT, spec), null, `${bad} must not resolve`);
  }
});

test('only own properties resolve, so an inherited key is not read as usage', () => {
  const spec = { kind: 'json', event: 'result', tokens_in: 'toString', tokens_out: 'toString' };
  assert.equal(parseCliUsage(CLAUDE_OUT, spec), null);
});
