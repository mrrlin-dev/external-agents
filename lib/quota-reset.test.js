...
test('"Resets in <number>" without a unit is refused, not read as seconds', () => {
  // Raised in review and reproduced. parseDurationToSeconds reads a bare number
  // as SECONDS — right for a retry-after header, dangerous here: "Resets in 4."
  // resolved to FOUR SECONDS, so a provider meaning four hours would have been
  // retried almost immediately, forever. The hand-rolled pattern this replaced
  // could not do it because it demanded an explicit `h`.
  const now = Date.now();
  const s = Math.floor(now / 1000);
  const fallback = s + 48 * 3600;
  for (const t of ['Resets in 4.', 'Resets in 30, try later.', 'Resets in 5)']) {
    assert.equal(
      resolveExhaustionResetAt({ text: `quota reached. ${t}`, provider: 'x', nowMs: now }),
      fallback,
      `${t} must fall through, not become a few seconds`,
    );
  }
  // And the forms the generalization exists to support still resolve.
  assert.equal(resolveExhaustionResetAt({ text: 'quota. Resets in 90s.', provider: 'x', nowMs: now }), s + 90);
  assert.equal(resolveExhaustionResetAt({ text: 'quota. Resets in 4h30m14s.', provider: 'x', nowMs: now }), s + 16214);
  assert.equal(resolveExhaustionResetAt({ text: 'quota. Resets in 1h30m.', provider: 'x', nowMs: now }), s + 5400);
});
