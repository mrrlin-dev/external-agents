...
function clauseAxis(clause, limitFrom, limitTo) {
  ...
  while ((m = re.exec(clause)) !== null) {
    ...
    const d = end <= limitFrom ? limitFrom - end : start - limitTo;
    if (d >= 0 && d < best) {
      best = d;
      nearest = (m[1] || "").toLowerCase() === "o" || (m[2] || "").toLowerCase() === "output"
        ? "output"
        : "input";
    }
  }
...
export function parseStatedLimit(text) {
  for (const clause of String(text || "").split(CLAUSE_SPLIT_RE)) {
    ...
    let m;
    while ((m = re.exec(clause)) !== null) {
      if (clauseAxis(clause, m.index, m.index + m[0].length) === "output") continue;
