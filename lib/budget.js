 ...
 *
 * But only when the sentence is about the INPUT. Groq writes three different
...
export function parseStatedLimit(text) {
  const s = String(text || "");
  // An output allowance teaches nothing about how large a prompt may be.
  if (/\bOTPM\b|\boutput tokens per minute\b/i.test(s)) return null;
  const m = /Limit\s+(\d+)\s*,\s*Requested\s+(\d+)/i.exec(s);
  if (!m) return null;
