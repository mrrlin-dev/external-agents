 ...
 *
 * HTTP only, and that restriction is load-bearing. On a CLI transport `tokens_in`
 * is whatever the CLI calls its input field, and for `claude --print` that field
 * is NOT the prompt: a measured run reported input_tokens 10 beside
 * cache_creation 16104 and cache_read 21675, so the number named "input" was
 * 0.03% of what was actually processed (see cli-usage.js). Feeding that to a
 * ceiling would be raising it on a number that does not mean what it says. An
 * input axis is only ever learned from HTTP prose in the first place, so nothing
 * is lost by declining the CLI case outright.
 */
export function raisedByAcceptedPrompt(observed, tokensIn, now = Math.floor(Date.now() / 1000), transport = null) {
  const limits = observed?.observed_limits;
  if (!limits?.axis_seen_at) return null;
  if (transport !== "generate_new") return null;
