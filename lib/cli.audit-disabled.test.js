...
test("audit --provider on a numbered id still covers the whole family, symmetrically", () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "ea-audit-family-numbered-"));
  try {
    seedClone(dir, { id: "groq-gpt-oss-120b-2", provider: "groq2" });
    const a = probedCount(runAudit(["--provider", "groq", "--json"], dir).stderr);
    const b = probedCount(runAudit(["--provider", "groq2", "--json"], dir).stderr);
    assert.equal(a, b, "a family is one target however it is named");
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
