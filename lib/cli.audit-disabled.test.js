...
test("audit --provider on a numbered id stays exact, and does not widen to the family", () => {
  ...
  try {
    ...
    // Stated rather than assumed: the comparison below is only meaningful while
    // the bundled registry still ships un-numbered groq seats for the family
    // name to reach. If that ever changes this fails loudly instead of quietly
    // passing on two equal counts.
    assert.ok(family >= 2, `bundled registry must ship un-numbered groq seats, saw ${family}`);
...
test("audit --provider never widens to the whole registry on a degenerate argument", () => {
  ...
  try {
    // "" reduces to the empty family too, and reaches the guard as a non-null
    // filter — the shape round 2 asked about explicitly.
    // "123" and a typo reduce to a family that matches nothing: exit 3.
    // "" cannot express intent at all, so it is a usage error: exit 2. What
    // matters for all three is that not one of them probes anything.
    for (const [arg, expected] of [["123", 3], ["", 2], ["nosuchprovider", 3]]) {
      const res = runAudit(["--provider", arg, "--json"], dir);
      assert.equal(res.status, expected, `--provider "${arg}" must match nothing, not everything`);
